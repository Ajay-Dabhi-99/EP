      ...
      <div className="space-y-8 h-full">
        ...
        <div className="space-y-2.5">
          {loading ?
           ...
           {!loading && invoice.docs?.length > 0 && ((invoice?.totalPages > 0) ? <InvoicePaggination invoice={invoice} limit={limit} setPageNo={setPageNo} pageNo={pageNo} /> : <h1 style={{ margin: "100px 0" }}>{intl.formatMessage({ id: "NO INVOICE FOUND" })}</h1>)}
        </div>
      </div>
    </div>
  )
